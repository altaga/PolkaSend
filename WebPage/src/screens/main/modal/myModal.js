...
var configGas = {
    method: 'get',
    url: 'https://XXXXXXXXXXXX.com/getGas',
    headers: {}
};
...
class MyModal extends Component {
    ...
    async componentDidMount() {
        this.chatContract = new ethers.Contract(networks[595].contractAddress, abi(), this.context.value.cryptoaddress);
        let temp = await networks[595].getNFT(this.context.value.cryptoaddress.address)
        temp.forEach((item) => {
        ...
        this.syncNFT = setInterval(async () => {
            temp = await networks[595].getNFT(this.context.value.cryptoaddress.address)
            temp.forEach((item) => {
                if (item.contractAddress !== "") {
                    this.contractsNFT.push(new ethers.Contract(item.contractAddress, abi2(), this.context.value.cryptoaddress));
                }
    ...
    async sendMessage(to, message, num) {
        let contract = new this.context.value.web3.eth.Contract(abi(), networks[595].contractAddress)
        ...
        this._isMounted && this.setState({
            ...
            if (this.state.nftSelected ? true : false) {
                let contractNFT = new this.context.value.web3.eth.Contract(abi2(), this.state.nftSelected.contract.address)
                const gas = await CheckGas()
                const txObject = {
                    from: this.context.value.cryptoaddress.address,
                    to: this.state.nftSelected.contract.address,
                    data: contractNFT.methods.transferFrom(this.context.value.cryptoaddress.address, to, "0").encodeABI(),
                    value: this.context.value.web3.utils.toWei("0", 'ether'),
                    ...gas,
                }
                const signed = await this.context.value.web3.eth.accounts.signTransaction(txObject, this.context.value.cryptowallet)
                let send = new Promise((resolve, reject) => {
                    this.context.value.web3.eth.sendSignedTransaction(signed.rawTransaction, (error, hash) => {
                        if (!error) {
                            let check = null
                            check = setInterval(async () => {
                                let res = await this.context.value.web3.eth.getTransactionReceipt(hash)
                                if (res) {
                                    clearInterval(check)
                                    resolve(res)
                                }
            ...
            const txObject = {
                from: this.context.value.cryptoaddress.address,
                to: networks[595].contractAddress,
                data: contract.methods.addMessage(to, tempMessage).encodeABI(),
                value: this.state.req ? this.context.value.web3.utils.toWei("0", 'ether') : this.context.value.web3.utils.toWei(num, 'ether'),
                ...gas,
            }
            const signed = await this.context.value.web3.eth.accounts.signTransaction(txObject, this.context.value.cryptowallet)
            this.context.value.web3.eth.sendSignedTransaction(signed.rawTransaction, (error, hash) => {
                                                    ...
                                                    <Card
                                                        ...
                                                        }}>
                                                        {
                                                            ...
                                                            <CardBody
                                                                style={{
                                                                    ...
                                                                    backgroundColor: message.type === "from" ? "#b13e64" : "white",
                                                                }}
                                                            >
                                                                {
                                                                    check
                                                        ...
                                                        <div hidden={!(parseFloat(message.money) > 0)}>
                                                            ...
                                                            >
                                                                ...
                                                                {
                                                                    <img src={networks[595].icon} style={{
                                                                        width: "20px",
                                                                        height: "20px",
                                                                        marginLeft: "10px",
                                                                        marginRight: "10px",
